...
#### 1. Para atender os resiquistos impostos pela US307, começamos por criar um Trigger responsável por calcular a ocupação de um Armazém sempre que um novo contentor é adicionado ou retirado de um armazém. Para isso, começamos por guardar na variável "destAux" o destino de um determinado contentor.
...
#### 3. Caso a variável corresponda ao nome de um aramazém, iremos armazenar o ID desse armázem na variável "dest".
...
#### 5. Para além,  de contabilizar os contentores que já se encontram no armazém, optamos por também contabilizar os contentores que tem como destino e chegam em breve aquele aramazém.
![image5.png](US307_md/image5.png)

#### 6. Após contabilizarmos todos os contentores, iremos selecionar a capacidade da warehouse com o ID igual ao da variável "dest" e faremos o cálculo da ocupação e armazenaremos na variável "ocRate". Após a realização do cálculo iremos verificar se a ocupação desse armazém é menor que 100% caso seja adcionamos os novos contentores e damos "UPDATE" a sua ocupação, caso contrário lançamos uma exceção para dizer que o armazém se encontra na sua ocupação máxima.![img.png](US307_md/image6.png) 
...
#### 3. Ao tentar adcionar outro contentor na "Warehouse3" receberemos um aviso e não será possível adcionar o novo contentor a este armazém visto que já está com a sua capacidade máxima preenchida 
